...
2. `mail_archive_root` `sharepoint_table_path` `sharepoint_case_root` を実環境に合わせて編集
3. Excel台帳を読む場合は `table.columns.*` を台帳の実列名に合わせて編集
4. `.\shinsa.bat` を起動し、最初に `sync` を実行

## データモデル

- `sync` は Mail Archive と OneDrive 同期済みローカルから `table.json` `mails.json` `folders.json` を再生成します
- `cache.json` は保持され、手動メール紐付けとUI状態を持ちます
- GUI は4つのJSONを読み込み、メモリ上で突合して表示します
- `writeback` は `table.json` と正本台帳との差分だけを反映します
...
- `data/sample/mail`
- `data/sample/onedrive/table/table.source.json`
